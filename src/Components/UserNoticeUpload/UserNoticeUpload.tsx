...
export default function UserNoticeUpload({
      ...
      if (routeData?.type === 'MODIFY') {
        ...
        try {
          ...
          const data = {
            notice_title: noticeData?.title,
            notice_body: noticeData?.desc,
            images: bufferResult?.map(({ name }: { name: string }) => name),
          }

          await postMutate
            ?.mutateAsync({ url: `/feaktion/${feaktion_id}/notice`, data })
            .then(result => {
              const { status } = result
              if (![200, 201].includes(status)) return
            })
  ...
  const dataSendHandler = async () => {
    if (noticeData?.title?.length <= 1) {
      setAlert({ type: 'title', text: '제목은 최소 2자 이상 입력해주세요' })
      return 'error'
    } else if (noticeData?.desc?.length <= 9) {
      setAlert({ type: 'desc', text: '내용은 최소 10자 이상 입력해주세요' })
      return 'error'
    } else {
      return 'ok'
    }
        ...
        <LayoutInner>
          {inputData?.map(({ index, value, type, placeHolder }) => (
            <TextInputWrap key={`keys__${index}`} os={Platform.OS}>
              <TextInput
                onChangeText={text => valueManufacturing(type, text)}
                value={value}
