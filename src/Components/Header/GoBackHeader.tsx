...
export default function GoBackHeader({
  ...
  const uploadBtn = ['UploadCoverEdit', 'FictionSceneModify', 'EditorWorkCheck']

  const searchBtn = ['uploadBtn', 'MyFictionList', 'FictionIndex', 'UserBoard']
  ...
  const sideButtonHandler = () => {
    if (['Notification', 'Comment'].includes(name)) {
      ...
      return (
        <>
          <FirstButton onPress={() => pressIconHandler('first', name)}>
            {firstButtonHandler()}
          </FirstButton>
  ...
  const firstButtonHandler = () => {
    if (name === 'MyBoard') {
      ...
      return <MoreMenu width={24} height={24} />
    } else if (['FictionEditorShort', 'FictionEditorSerise'].includes(name)) {
