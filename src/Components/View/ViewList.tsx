...
type props = {
  ...
  currentType: string
  currentView: number
  setLayoutheight: Dispatch<SetStateAction<{}[]>>
  layoutheight: {}[]
}
...
export default function ViewList({
  ...
  useEffect(() => {
    ...
    if (currentView === _index) {
      opacityImage.value = withDelay(400, withTiming(1, { duration: 400 }))
      opacityText.value = withDelay(1300, withTiming(1, { duration: 400 }))
    }
  }, [currentView])
  ...
  const titleHandler = () => {
    ...
    return (
      <ContentLayout height={windowHeight}>
        <LayoutContainer>
          <Animated.View style={[styles.animationStyle, animatedDesc]}>
            {divideType ? (
              <EpisodeTitle styling={adjustViewText?.backcolor}>
                {epititle}
              </EpisodeTitle>
            ) : null}
            <TitleText styling={adjustViewText?.backcolor}>{title}</TitleText>
          </Animated.View>
        </LayoutContainer>
      </ContentLayout>
    )
...
const DescText = styled.Text<{ fontCtrl: adjustText }>`
  ...
  color: ${({ theme, fontCtrl }) =>
    fontCtrl?.backcolor === 'image' ? theme.color.gray3 : theme.color.gray1};
...
const TextDesc = styled.View<{
  ...
  height: 100%;
  width: 100%;
  background: ${({ fontCtrl, noBackground, theme }) => {
    if (noBackground) {
