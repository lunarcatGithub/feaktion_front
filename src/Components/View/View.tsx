...
export default function View({ navigation, route }: any) {
  // store
  const { setEpisodePostId, episodePostId, adjustViewText, setAdjustViewText } =
    useViewerContext();

  // route
  const { currentType, fictionId, episodeId, previewData, scroll, isUploaded } =
    route?.params;
  ...
  // fetch
  const fictionData = useQuery(
    ['fiction', currentFictionId],
    () =>
      useFetch({
        url: `/feaktion/${fictionId}`,
        method: 'get',
      }),
    { retry: true, enabled: currentType !== 'Preview' },
  );
  const fictions = fictionData?.data?.data?.data; // cover data만 추출
    ...
    <>
      {currentType !== 'Preview' ? (
        <ViewContent
          ...
          episodeId={currentPage === 'Short' ? episodeId : currentEpisodeId}
          scroll={currentScroll}
          // isMe={fictions?.isWriter}
          data={episodeData?.data?.data?.data}
