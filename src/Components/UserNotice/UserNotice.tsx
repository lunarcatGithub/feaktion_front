...
export default function UserNotice({ navigation, route }: any): JSX.Element {
  ...
  const { data } = useQuery(
    [
      'Notice',
      { fiction: currentNotice?.fictionId, notice: currentNotice?.noticeId },
    ],
    () =>
      useFetch({
        url,
  ...
  const modalHanddler = (modalType: string, select: string): void => {
    if (select === 'confirm') {
      RemoveMutate.mutateAsync({
        ...
        queryClient.invalidateQueries(['fiction', noticeData?.feaktion_id])
      })
    }
    setIsPopup(false)
  }
  ...
  return (
    <>
      <Layout>
        <HeaderWrap onLayout={getSizeLayout}>
          <Header
            ...
            route={{ name: 'UserNotice', params: { title: '공지사항' } }}
      ...
      />
      <Dropdown
        data={currentNotice?.isWrite ? isMeNoticeDropDown : isNotMedropdownList}
        visible={isModal}
