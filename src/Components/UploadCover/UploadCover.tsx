...
import { UploadCoverDataStore } from './UploadCoverDataStore'
import { getUploadedCoverAgent } from '~/Agent/FeaktionAgent'
...
export default function UploadCover({ navigation, route }: any): JSX.Element {
    ...
    // data
    const [currentRoute] = useState(route?.params.feaktionType)
    ...
    // All data handler
    useEffect(() => {
        if (['short', 'fiction'].includes(currentRoute)) {
            const uploadCoverData = UploadCoverDataStore()
            // 데이터 초기화
            if (!uploadCoverData) return
            uploadCoverData.init()
            setCoverData(uploadCoverData)
        }
    }, [currentRoute])
    ...
    const uploadCoverData = (type: string) => {
        if (type === 'goback') {
            navigation.goBack()
        ...
        const genlist = selectGenre?.map(
            ({ value }: { value: string }) => value
        )

        if (['shortModify', 'short'].includes(currentRoute)) {
            if (selectGenre === null || selectGenre?.length === 0) {
                setAlertText({
                    type: 'genre',
            ...
            // 수정 정보가 있는 경우 (수정하기)
            if (['shortModify', 'fictionModify'].includes(currentRoute)) {
                navigation.navigate('UploadFiction', {
                    screen: 'FictionEditor',
                    params: {
                        // fictionId: JSON.stringify(0),
    ...
    const data = getUploadedCoverAgent({
        fictionId: currentRoute?.fictionId,
        option: { enabled: currentRoute?.fictionId ? true : false },
    })
    ...
    const customArr = [
        // custom big button
        {
            ...
            alertType: 'genre',
            _title:
                selectGenre?.map(({ title }) => title)?.join(' ,  ') ||
                '장르 선택',
            _type: 'Arrow',
    ...
    const tagsHandler = (type?: number | null): void => {
        // if modified tags
    ...
    const optionButton = [
        {
            type: 'nextScreen',
            button: <HeaderTextButton>다음</HeaderTextButton>,
            method: () => uploadCoverData('nextScreen'),
        },
    ]
...
const HeaderTextButton = styled.Text<{ type?: string }>`
    color: ${({ theme, type }) =>
...
// const Text = styled.Text<{ type?: string }>`
//   color: ${({ theme, type }) =>
//     type === 'active' ? theme.color.purple4 : theme.color.gray3};
//   font-size: ${({ theme }) => theme.fontSize.font14};
//   font-family: ${({ theme }) => theme.font.notoMedium};
