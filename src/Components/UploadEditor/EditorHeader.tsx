...
export default function EditorHeader({
    ...
    const moveChckStackHandler = (): void => {
        if (
            fictionData?.episodeTitle?.length === 0 &&
            currentRoute !== 'SHORT'
        ) {
            ...
            (fictionData?.scene[0].desc === '' &&
                fictionData?.scene[0].index === 0)
        ) {
    ...
    const titleHandler = () => {
        if (currentRoute === 'SHORT') {
            ...
            ['NovelSaved', 'FictionEditorSerise'].includes(currentRoute)
        ) {
            setTitleName({
                name: currentRoute,
    ...
    const optionButton = [
        {
            type: 'nextScreen',
            button: <HeaderTextButton>임시저장</HeaderTextButton>,
            method: () => {},
        },
        {
            type: 'nextScreen',
            button: <HeaderTextButton>다음</HeaderTextButton>,
            method: moveChckStackHandler,
        },
    ]
