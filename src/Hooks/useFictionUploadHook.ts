...
function useFictionUploadHook(): [
  ...
  const postOrPatchMutate = useMutationHook(
    ['FictionModify', 'ShortModify'].includes(currentType) ? 'patch' : 'post'
  )
  ...
  const imageHandler = async () => {
    ...
    const _bufferResult = await bufferHandler([
      { index: 0, uri: image?.uri, base64: image?.base64, type: image?.type },
    ])
  ...
  const dataHandler = async ({
    ...
    const handleType = ['FictionModify', 'ShortModify'].includes(currentType)
      ? 'MODIFY'
      : 'NEW'
    ...
    return postOrPatchMutate
      ...
      .then((data: any) => {
        if ([200, 201].includes(data?.status)) {
          ...
          setUploadFictionId(JSON.stringify(data?.data?.data?.feaktion_id))
        ...
        if (
          ...
          navigation.navigate('SideStack', {
            ...
            params: {
              ...
              currentType: currentType === 'Short' ? 'Short' : 'Story',
              isUploaded: currentType,
            },
          })
        }
